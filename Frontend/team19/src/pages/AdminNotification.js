...
const {admin}=useAuthContext2()
  ...
  useEffect(() => {
    const fetchNotifi = async () => {
      const response = await fetch('http://35.244.31.186:8080/api/user/notification')
  ...
  const handleClick = async (not) => {
    ...
    const response = await fetch(' http://35.244.31.186:8080/api/admin/delnotification/' + not._id, {
      method: 'DELETE',
      headers: {
